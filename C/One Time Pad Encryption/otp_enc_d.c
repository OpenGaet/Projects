...
// Sigterm handler kills all child processes
  ...
  while (1)
  {
    ...
    if (pool[j] == 0)
    {
      ...
      while (totalRecd < totalToRec)
      {
        ...
        // Concatenate buffer to the plaintext string
        i = totalRecd;
      ...
      while (totalSent < totalToSend)
      {
        ...
        // Send chunks of the encrypted text
    ...
    // Large, lazy limit for processes
    j++;
    if (j == 1000)
      error("ERROR: too many process\n");
